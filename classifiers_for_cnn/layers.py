...
def batchnorm_forward(x, gamma, beta, bn_param):
  ...
  if mode == 'train':
    ...
    cache = (beta, gamma, mean, variance, eps, normalized_x, x)
...
def batchnorm_backward(dout, cache):
  ...
  # ================================================================ #
  beta, gamma, mu, var, eps, normalized_x, x = cache
  N = dout.shape[0]
  dbeta = np.sum(dout, axis = 0)
  dgamma = np.sum(dout * normalized_x, axis = 0)
  dnorm_x = dout*gamma
  dvar = np.sum(-0.5*dnorm_x*(x - mu)*(var + eps)**(-3/2), axis = 0) 
  dmu = np.sum(-dnorm_x/np.sqrt(var + eps), axis = 0) + dvar*(np.sum(-2*(x-mu), axis = 0)/N)
  dx = dnorm_x/np.sqrt(var + eps) + dvar*2*(x - mu)/N + dmu/N
  # ================================================================ #
  # END YOUR CODE HERE
  # ================================================================ #
