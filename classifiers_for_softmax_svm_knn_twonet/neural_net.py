...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    hidden = scores_first*(scores_first>0)
    ...
    # Notes from mark: You forgot to add the loss from the bias b1 and b2
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    ...
    probs = np.exp(scores)/denominator
    dW2 = hidden.T @ probs -  hidden.T @ y_matrix
    db2 = np.sum(probs - y_matrix, axis = 0)
    #import pdb; pdb.set_trace()
    dhR2 = probs.dot(W2.T) - y_matrix.dot(W2.T)
    dhR1 = (hidden>0)*dhR2
    
    h_matrix = np.zeros((num_train,hidden.shape[1]))
    h_matrix[np.arange(num_train),y] = 1 
    dW1 = X.T.dot(dhR1)
    db1 = np.sum(dhR1, axis=0)    
  ...
  def train(self, X, y, X_val, y_val,
    ...
    for it in range(num_iters):
      ...
      #########################################################################
      index_train = np.random.choice(num_train, batch_size)
