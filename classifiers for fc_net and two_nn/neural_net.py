...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    scores_first = X.dot(W1.T) + b1
    hidden = scores_first*(scores_first>0)
    scores = hidden.dot(W2.T) + b2
    ...
    dscores =  np.exp(scores)/np.sum(np.exp(scores),axis = 1, keepdims=True) - y_matrix
    dW2 = dscores.T.dot(hidden)
    db2 = np.sum(dscores, axis = 0)
    drelu = (dscores.dot(W2))*(hidden>0)
    dW1 = drelu.T.dot(X)
